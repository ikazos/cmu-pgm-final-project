...
from naive_bayes_multivariate_factored import MultivariateFactoredNB
from dimension_correlated_nb import DimensionCorrelatedNB
...
def setup(rawdata, dataname, num_epochs):
    ...
    priors = torch.ones(num_relations).to(device) # using 1's because Varun has an assertion that priors are not 0.
    # normal_nb = NaiveBayes(emb_dim, num_entities, num_relations, priors)
    # normal_nb = DimensionCorrelatedNB(emb_dim, num_entities, num_relations, priors)
    normal_nb = MultivariateFactoredNB(
    #     # emb_dim, min(num_relations, 10), num_entities,
        emb_dim, num_relations, num_entities,
        num_relations, priors, train_prior=False
    )
    normal_nb.to(device)
...
NUM_EPOCHS = 10
HEADERS = ( "epoch", "acc", "f1", "loss", "set", "emb", "data" )
